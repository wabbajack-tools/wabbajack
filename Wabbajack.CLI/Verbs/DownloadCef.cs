...
namespace Wabbajack.CLI.Verbs
{
    public class DownloadCef : IVerb
    {
        ...
        public DownloadCef(ILogger<DownloadCef> logger, DownloadDispatcher dispatcher, FileExtractor.FileExtractor fileExtractor, HttpClient httpClient)
        {
            _logger = logger;
...

            {
                ...
                while (true)
                {
                    ...
                    if (path.InFolder(prefix) && entry.Size > 0)
                    {
                        ...
                        _logger.LogInformation("Extracting {FileName} to {Folder}", outputPath.FileName, outputPath.RelativeTo(folder));
