...
/// <summary>
/// Interaction logic for CompilerDetailsView.axaml
/// </summary>
public partial class CompilerDetailsView : ReactiveUserControl<CompilerDetailsVM>
{
    ...
    // NOTE: the four Add*Command methods below are ported verbatim. In the original WPF view they are
    // not wired to any control in CompilerDetailsView.xaml either (no Command/Click reference them), so
    // they are preserved as-is for parity; nothing in this port changes their (pre-existing) disconnected
    // status. They rely on Microsoft.WindowsAPICodePack.Dialogs (a native Win32 folder/file picker), which
    // is UI-framework agnostic and works unchanged under Avalonia on Windows -- see todos re: package ref.
