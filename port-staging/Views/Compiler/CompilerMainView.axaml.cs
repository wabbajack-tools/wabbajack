...
public partial class CompilerMainView : ReactiveUserControl<CompilerMainVM>
{
    ...
    public CompilerMainView()
    {
        ...
        this.WhenActivated(disposables =>
        {
            ...
            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Configuration)
                .BindToStrict(this, view => view.CompilerDetailsView.IsVisible)
                .DisposeWith(disposables);

            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Configuration)
                .BindToStrict(this, view => view.FileManager.IsVisible)
                .DisposeWith(disposables);

            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Configuration)
                .BindToStrict(this, view => view.ConfigurationButtons.IsVisible)
                .DisposeWith(disposables);

            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Compiling || s == CompilerState.Errored)
                .BindToStrict(this, x => x.LogView.IsVisible)
                .DisposeWith(disposables);

            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Compiling)
                .BindToStrict(this, x => x.CpuView.IsVisible)
                .DisposeWith(disposables);

            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Compiling)
                .BindToStrict(this, view => view.CompilationButtons.IsVisible)
                .DisposeWith(disposables);
            ...
            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Completed)
                .BindToStrict(this, view => view.CompiledImage.IsVisible)
                .DisposeWith(disposables);

            ViewModel.WhenAny(vm => vm.State)
                .Select(s => s == CompilerState.Completed)
                .BindToStrict(this, view => view.CompletedButtons.IsVisible)
                .DisposeWith(disposables);
            ...
            ViewModel.StartCommand.Events().CanExecuteChanged
                .Subscribe(_ =>
                {
                    ...
                    {
                        ToolTip.SetTip(StartButton, "Cannot start compilation, not all required fields have been filled out.");
                    }
                    else ToolTip.SetTip(StartButton, null);
                })
                .DisposeWith(disposables);
            ...
            ViewModel.WhenAnyValue(vm => vm.PreflightChecksPassed)
                .Select(passed => passed == false)
                .BindToStrict(this, v => v.RefreshPreflightButton.IsVisible)
                .DisposeWith(disposables);
            ...
            ViewModel.WhenAnyValue(vm => vm.State)
                .Select(s => s == CompilerState.Completed)
                .BindToStrict(this, v => v.PreflightStatusText.IsVisible)
                .DisposeWith(disposables);
            ...
            ViewModel.WhenAnyValue(vm => vm.IsBusy)
                .BindToStrict(this, v => v.BusyOverlay.IsVisible)
                .DisposeWith(disposables);
    ...
    private async Task HandlePublishCollectionClick()
    {
        ...
        var result = await ShowYesNoDialogAsync(
    ...
    // TODO: WPF's System.Windows.MessageBox has no Avalonia equivalent (no MsBox.Avalonia/similar
    // dependency was available to add under the read-only port constraints). This is a minimal
    // faithful stand-in: a small owned Window with the message text and Yes/No buttons, returning
    // true only on "Yes" - matching the original's MessageBoxButton.YesNo / MessageBoxResult.Yes check.
    private async Task<bool> ShowYesNoDialogAsync(string message, string title)
    {
        var owner = TopLevel.GetTopLevel(this) as Window;

        var tcs = new TaskCompletionSource<bool>();

        var yesButton = new Button { Content = "Yes", MinWidth = 80, Margin = new Thickness(0, 0, 8, 0) };
