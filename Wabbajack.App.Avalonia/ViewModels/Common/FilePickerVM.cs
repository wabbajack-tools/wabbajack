// Copied from the WPF app, which builds without nullable annotations.
...
using DialogService = Wabbajack.App.Avalonia.Services.FilePicker;

namespace Wabbajack.App.Avalonia.ViewModels.Common;

/// <summary>
/// The WPF app's FilePickerVM, with the same public shape: a target path that is checked for existence every
/// few seconds and against the filters, an error and tooltip derived from both, and a command that opens the
/// system dialog. The one change is the dialog, which goes through <see cref="DialogService" /> and is
/// asynchronous, so <see cref="SetTargetPathCommand" /> is a <c>ReactiveCommand&lt;Unit, Unit&gt;</c> that
/// completes once the dialog closes, as the WPF one (synchronous) did.
/// </summary>
public partial class FilePickerVM : ViewModel
{
    ...
    // Stateless: it reaches the main window's StorageProvider itself.
    private static readonly DialogService Dialogs = new();
    ...
    public ICommand ConstructTypicalPickerCommand(IObservable<bool> canExecute = null)
    {
        return ReactiveCommand.CreateFromTask(
            execute: async () =>
            {
                // The WPF dialog also opened in TargetPath's folder; DialogService takes no starting folder.
                AbsolutePath path;
                if (PathType == PathTypeOptions.Folder)
                {
                    path = await Dialogs.PickFolder(PromptTitle);
                }
                else
                {
                    var filters = Filters.Items
                        .SelectMany(f => f.Patterns.Select(p => (f.DisplayName, p)))
                        .ToArray();
                    path = await Dialogs.PickFile(PromptTitle, filters);
                }
