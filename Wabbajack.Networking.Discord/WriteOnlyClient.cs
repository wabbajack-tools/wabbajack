...
namespace Wabbajack.Networking.Discord
{
    public class WriteOnlyClient
    {
        ...
        public WriteOnlyClient(ILogger<WriteOnlyClient> logger, ITokenProvider<Dictionary<Channel, DiscordWebHookSetting>> token, HttpClient client, IResource<HttpClient> limiter)
        {
            _logger = logger;
            _token = token;
            _httpClient = client;
