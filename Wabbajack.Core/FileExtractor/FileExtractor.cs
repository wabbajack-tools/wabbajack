...
public class FileExtractor
{
        ...
        Action<Percent>? progressFunction = null)
    {
        ...
        try
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                initialPath = @"FileExtractor\Extractors\windows-x64\7z.exe";
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                initialPath = @"FileExtractor\Extractors\linux-x64\7zz";
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                initialPath = @"FileExtractor\Extractors\mac\7zz";
        ...
        Action<Percent>? progressFunction = null)
    {
        ...
        try
        {
            ...
            var initialPath = "";
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                initialPath = @"FileExtractor\Extractors\windows-x64\innoextract.exe";
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                initialPath = @"FileExtractor\Extractors\linux-x64\innoextract";
